...
function initMsg(flag) {
    ...
    $.ajax({
        ...
        var item = data.FormDataSet.BPM_YYZPJTAQDQJCFK_A[0];
        ...
        $("#fjcly").val(item.fjcly);
        $("#fifyyh").val(item.fifyyh);
        var fjclystr = String(item.fjcly).split(",");
        for (i = 0; i < fjclystr.length; i++){
            if (fjclystr[i] == "月度综合检查") {
                $("#fydzhjc").attr("checked", "checked");
            } else if (fjclystr[i] == "节假日检查") {
                $("#fjjrjc").attr("checked", "checked");
            } else if (fjclystr[i] == "季节检查") {
                $("#fjjjc").attr("checked", "checked");
            } else if (fjclystr[i] == "专项检查") {
                $("#fzxjc").attr("checked", "checked");
            }
        }
        if (item.fifyyh == "是") {
            $("#jclist").css("display", "block");
        }
        var bli = data.FormDataSet.BPM_YYZPJTAQDQJCFK_B;
...
function nodeController(data) {
    ...
    if (NodeName == "开始") {
        ...
        if (typeof (NodeName) != "undefined") {
            ...
            if (NodeName == "分公司分管经理" || NodeName == "子公司安全员" || NodeName == "子公司安全负责人") {
                $("#approvalD").empty();
                var li = '&nbsp;&nbsp;';
...
function hasRead() {
    ...
    $("#jclist").find("#bl").each(function () {
        ...
        var fwwczgyy = $(this).find("#fwwczgyy").val();
    ...
    mui.prompt('请选填知会意见', '可以不填', '知会意见', btnArray, function (e) {
        if (e.index == 1) {
            ...
            for (var i = 0; i < bllistArr.length; i++) {
                ...
                xml = xml + '<fwwczgyy>' + bllistArr[i].fwwczgyy + '</fwwczgyy>';
