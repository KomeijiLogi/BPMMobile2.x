function prepMsg() {
    ...
    xml = xml + '       <ProcessName>海盛公司部门月度办公用品申请</ProcessName>';
...
function initList() {
    ...
    xml = xml + '         <ID>erpcloud_海盛公司部门月度办公用品申请</ID>';
    xml = xml + '         <Type>1</Type>';
    xml = xml + '        <Method>GetUserDataProcedure</Method>';
    xml = xml + '        <ProcedureName>erpcloud_海盛公司部门月度办公用品申请</ProcedureName>';
    xml = xml + '        <Filter>';
    xml = xml + '        </Filter>';
    xml = xml + '      </Params>';
    xml = xml + '   </Requests>';
    var initMainMsg = $.ajax({
        ...
        for (var i = 0; i < dataArr.length;i++){
            ...
            li = li + '<input type="radio" name="radio" data-fwpno="' + dataArr[i].fwpno + '"';
            li = li + 'data-fwpname="' + dataArr[i].fwpname + '"';
            li = li + 'data-fggxh="' + dataArr[i].fggxh + '"';
            li = li + 'data-funit="' + dataArr[i].funit + '"';
            li = li + ' />' + dataArr[i].fwpname + '&nbsp' + dataArr[i].fggxh + '&nbsp' + dataArr[i].funit ;
            li = li + '</li>';
            $("#cardlist").append(li);
        }
        ...
        //done event
            ...
            if (checkedValues.length > 0) {
                for (var i = 0; i < checkedElements.length;i++){
                    ...
                    li = li + '      <label for="fwpno">物料编码<i style="color:red;">*</i></label>';
                    ...
                    li = li + '      <input type="text" id="funit" name="funit" readonly="readonly" value="' + checkedElements[i].funit + '"/>';
                    li = li + '   </div>';
                    li = li + '   <div class="mui-input-row">';
                    li = li + '      <label for="famount">申请数量<i style="color:red;">*</i></label>';
                    li = li + '      <input type="number" id="famount" name="famount" placeholder="请填写申请数量"/>';
                    li = li + '   </div>';
                    li = li + '   <div class="mui-input-row">';
...
function mxItem(fwpno, fwpname, fggxh, funit, famount, fremark) {
...
function initData(data, flag) {
    var item = data.FormDataSet.BPM_HSYDBGYPSQ_A[0];
    if (flag) {
    ...
    var item_c = data.FormDataSet.BPM_HSYDBGYPSQ_B;
    for (var i = 0; i < item_c.length; i++){
        ...
        li = li + '   <div class="mui-input-row">';
        li = li + '      <label for="famount">申请数量<i style="color:red;">*</i></label>';
        li = li + '      <input type="number" id="famount" name="famount" readonly="readonly" value="' + item_c[i].famount + '"/>';
        li = li + '   </div>';
        li = li + '   <div class="mui-input-row">';
        li = li + '      <label for="fremark">备注</label>';
...
function nodeControllerExp(NodeName) {
    if (NodeName == '开始') {
        ...
        $('#tjmx').show();
        $("#mxlist").find("#famount,#fremark").each(function () {
...
function Save() {
    ...
    $("#mxlist").find("#mx").each(function () {
        ...
        if (mxItem(fwpno, fwpname, fggxh, funit, famount, fremark) == null) {
            mxflag = true;
            return;
        }
        var mx = mxItem(fwpno, fwpname, fggxh, funit, famount, fremark);
        mxlistArr.push(mx);
    });
    if (mxflag) {
        return;
