...
function prepMsg() {
    ...
    xml = xml + '       <ProcessName>建设公司其他费用报销</ProcessName>';
...
function initExpense(fno) {
    ...
    xml = xml + '         <ID>erpcloud_建设公司其他费用报销</ID>';
    xml = xml + '         <Type>1</Type>';
    xml = xml + '        <Method>GetUserDataProcedure</Method>';
    xml = xml + '        <ProcedureName>erpcloud_建设公司其他费用报销</ProcedureName>';
    ...
    var initExpenseAjax = $.ajax({
        ...
        //done event
            ...
            if (checkedValues.length > 0) {
                ...
                hiddenCard();
                $("#fqtfysqd_taskid").val(checkedElements[checkedElements.length - 1].tid);
                $("#fqtfysqd").val(checkedElements[checkedElements.length - 1].fbillno);
...
function getList() {
    var fsqdID = $("#fqtfysqd_taskid").val();
    console.log(fsqdID);
    var xml = '<?xml version= "1.0" ?>';
    xml = xml + ' <Requests>';
    xml = xml + ' <Params>';
    xml = xml + '      <DataSource>BPM_EXPENSE</DataSource>';
    xml = xml + '      <ID>p_建设其他费用报销</ID>';
    xml = xml + '      <Type>2</Type>';
    xml = xml + '       <Method>GetUserDataProcedure</Method>';
    xml = xml + '      <ProcedureName>p_建设其他费用报销</ProcedureName>';
    xml = xml + '      <Filter>';
    ...
    $.ajax({
        ...
        for (var i = 0; i < listArr.length; i++) {
            ...
            li = li + '    </div>';
            li = li + '     <div class="mui-input-row">';
            li = li + '        <label for="fcsq">超申请情况%</label>';
            li = li + '        <input type="text" id="fcsq" name="fcsq" readonly="readonly" value="未超支"/>';
            li = li + '     </div>';
...
function calcPercent(context) {
    var fcsq = '未超支';
    var fsqje = parseFloat($(context).parent().parent().find("#fsqje").val());
    var fbxje = parseFloat($(context).parent().parent().find("#fbxje").val());
    if (!isNaN(fsqje) && !isNaN(fbxje) && fbxje > fsqje) {
        fcsq=  parseFloat((fbxje - fsqje) / fsqje *100).toFixed(2);
    }
...
function mxItem(ffyxm, fsqje, fbxje, fcsq, fgzcx) {
...
function initMsg(flag) {
    ...
    $.ajax({
        ...
        var item = data.FormDataSet.建设公司_其他费用报销_主表[0];
        if (flag) {
        ...
        $("#fdept").val(item.部门名称);
        $("#fcompany").val(item.公司名称);
        $("#fqtfysqd_taskid").val(item.其他费用申请单TaskID);
        $("#fqtfysqd").val(item.其他费用申请单号);
        $("#fsqje").val(item.申请金额);
        $("#fjkje").val(item.借款金额);
        ...
        var item_c = data.FormDataSet.建设公司_其他费用报销_子表;
        for (var i = 0; i < item_c.length; i++) {
            ...
            li = li + '     </div>'; 
            li = li + '     <div class="mui-input-row">';
            li = li + '        <label for="fcsq">超申请</label>';
            li = li + '        <input type="text" id="fcsq" name="fcsq" readonly="readonly" value="' + item_c[i].超申请 + '"/>';
            li = li + '     </div>';
            li = li + '     <div class="mui-input-row">';
            li = li + '         <label for="fgzcx">工作成效<i style="color:red;">*</i></label>';
            li = li + '         <input type="text" id="fgzcx" name="fgzcx" readonly="readonly" value="' + item_c[i].工作成效 + '"/>'; 
            li = li + '     </div>';
            li = li + '</div>';
            $("#mxlist").append(li);
        }
...
function Save() {
    ...
    var fqtfysqd = $("#fqtfysqd").val();
    var fqtfysqd_taskid = $("#fqtfysqd_taskid").val();
    var fsqje = $("#fsqje").val();
    var fjkje = $("#fjkje").val();
    ...
    $("#mxlist").find("#mx").each(function () {
        ...
        var fcsq = $(this).find("#fcsq").val();
        var fgzcx = $(this).find("#fgzcx").val();

        if (mxItem(ffyxm, fsqje, fbxje, fcsq, fgzcx) == null) {
            mxflag = true;
            return;
        }
        var mx = mxItem(ffyxm, fsqje, fbxje, fcsq, fgzcx);
        mxlistArr.push(mx);
    });
    if (mxflag) {
        return;
    ...
    mui.confirm('即将提交，是否确定？', '提交确认提醒', btnArry, function (e) {
        if (e.index == 1) {
            ...
            xml = xml + ' <其他费用申请单号>' + fqtfysqd + '</其他费用申请单号>';
            xml = xml + '  <其他费用申请单TaskID>' + fqtfysqd_taskid + '</其他费用申请单TaskID>';
            ...
            for (var i = 0; i < mxlistArr.length; i++) {
                ...
                xml = xml + '  <超申请>' + mxlistArr[i].fcsq + '</超申请>';
