...
function prepMsg() {
    ...
    xml = xml + '       <ProcessName>建设公司市场宣传品制作申请</ProcessName>';
...
function tapEvent() {
    ...
    $('#tjmx').on('tap', function () {
        ...
        li = li + '       <label for="fpm">品名<i style="color:red;">*</i></label>';
        li = li + '       <input type="text" id="fpm" name="fpm" placeholder="请填写品名"/>';
        li = li + '    </div>';
        li = li + '    <div class="mui-input-row">';
        ...
        li = li + '         <input type="text" id="fje" name="fje" readonly="readonly" value="0.00"/>';
        li = li + '    </div>';
        li = li + '    <div class="mui-input-row">';
        li = li + '         <label for="fgys">供应商<i style="color:red;">*</i></label>';
        li = li + '         <input type="text"  id="fgys" name="fgys" placeholder="请填写供应商" />';
        li = li + '    </div>';
        li = li + '</div>';
        $("#mxlist").append(li);
...
function amountPrice() {
    ...
    $("#mxlist").find("#mx").each(function () {
        ...
        var fsl = parseFloat($(this).find("#fsl").val());
        if (isNaN(fsl)) {
            fsl = 0;
        }
        sltotal = parseFloat(sltotal) + fsl;
    });
    $("#fsltotal").val(sltotal);
}
...
//将子表属性封装成一个对象，并添加子表验证
function mxItem(fpm, fgg, fdj, fsl, fje, fgys) {
...
function initMsg(flag) {
    ...
    $.ajax({
        ...
        var item = data.FormDataSet.建设公司_市场宣传品制作申请_主表[0];
        ...
        $("#fyt").val(item.用途);
        $("#fbz").val(item.备注);
        $("#fsltotal").val(item.合计数量);
        $("#ftotal").val(item.合计金额);

        var item_c = data.FormDataSet.建设公司_市场宣传品制作申请_子表;
        for (var i = 0; i < item_c.length; i++){
            ...
            li = li + '    <div class="mui-input-row">';
            li = li + '       <label for="fpm">品名<i style="color:red;">*</i></label>';
            li = li + '       <input type="text" id="fpm" name="fpm" readonly="readonly" value="' + item_c[i].品名 + '"/>';
            li = li + '    </div>';
            li = li + '    <div class="mui-input-row">';
            li = li + '        <label for="fgg">规格<i style="color:red;">*</i></label>';
            li = li + '        <input type="text" id="fgg" name="fgg" readonly="readonly" value="' + item_c[i].规格 + '"/>';
            li = li + '    </div>';
            li = li + '    <div class="mui-input-row">';
            li = li + '         <label for="fdj">单价<i style="color:red;">*</i></label>';
            ...
            li = li + '    <div class="mui-input-row">';
            li = li + '         <label for="fje">金额</label>';
            li = li + '         <input type="text" id="fje" name="fje" readonly="readonly" value="' + item_c[i].金额 + '"/>';
            li = li + '    </div>';
            li = li + '    <div class="mui-input-row">';
            li = li + '         <label for="fgys">供应商<i style="color:red;">*</i></label>';
            li = li + '         <input type="text"  id="fgys" name="fgys" readonly="readonly" value="' + item_c[i].供应商 + '"/>';
            li = li + '    </div>';
            li = li + '</div>';
            $("#mxlist").append(li);
            if (flag) {
...
function nodeController(data) {
    ...
    if (NodeName == "开始") {
        ...
        $("#mxlist").find("#fpm,#fgg,#fdj,#fsl,#fgys").each(function () {
...
function Save() {
    ...
    $("#mxlist").find("#mx").each(function () {
        ...
        if (mxItem(fpm, fgg, fdj, fsl, fje, fgys) == null) {
            console.log(mxItem(fpm, fgg, fdj, fsl, fje, fgys));
            mxflag = true;
            return;
        }
        var mx = mxItem(fpm, fgg, fdj, fsl, fje, fgys);
        mxlistArr.push(mx);
    });
    console.log(mxflag);
    if (mxflag) {
    ...
    mui.confirm('即将提交，是否确定？', '提交确认提醒', btnArry, function (e) {
        if (e.index == 1) {
            ...
            xml = xml + '       <ProcessName>建设公司VI制作申请</ProcessName>';
            xml = xml + '         <ProcessVersion>' + version + '</ProcessVersion>';
            ...
            for (var i = 0; i < mxlistArr.length;i++){
                xml = xml + '   <建设公司_市场宣传品制作申请_子表>';
                xml = xml + '  <RelationRowGuid>'+(i+1)+'</RelationRowGuid>';
                xml = xml + '   <RowPrimaryKeys></RowPrimaryKeys>';
                xml = xml + '   <序号>' + (i + 1) + '</序号>';
                xml = xml + '   <品名>' + mxlistArr[i].fpm + '</品名>';
                xml = xml + '  <规格>' + mxlistArr[i].fgg + '</规格>';
                xml = xml + '   <单价>' + mxlistArr[i].fdj + '</单价>';
                xml = xml + '   <数量>' + mxlistArr[i].fsl + '</数量>';
                xml = xml + '   <金额>' + mxlistArr[i].fje + '</金额>';
                xml = xml + '   <供应商>' + mxlistArr[i].fgys + '</供应商>';
                xml = xml + '  </建设公司_市场宣传品制作申请_子表>';
            }
