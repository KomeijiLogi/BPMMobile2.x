...
function AgreeOrConSign() {
    ...
    if (consignFlag) {
        consignAjax.then(function () {
            ...
            <报销日期>${item.报销日期}</报销日期>
            <车牌号>${item.车牌号}</车牌号>
            <月度预算金额>${changeNullToEmpty(item.月度预算金额)}</月度预算金额>
            <可用预算金额>${changeNullToEmpty(item.可用预算金额)}</可用预算金额>
            <费用属性>${item.费用属性}</费用属性>
            <超预算情况>${item.超预算情况}</超预算情况>
            <月度报销标准>${item.月度报销标准}</月度报销标准>
            <年度可报销月数>${item.年度可报销月数}</年度可报销月数>
            <年度报销标准>${changeNullToEmpty(item.年度报销标准)}</年度报销标准>
            <年度已报销金额>${changeNullToEmpty(item.年度已报销金额)}</年度已报销金额>
            <超标准情况>${item.超标准情况}</超标准情况>
            <车辆保险费报销金额>${changeNullToEmpty(item.车辆保险费报销金额)}</车辆保险费报销金额>
            <备注>${item.备注}</备注>
            <车辆燃油费报销金额>${changeNullToEmpty(item.车辆燃油费报销金额)}</车辆燃油费报销金额>
            <车辆修理费报销金额>${changeNullToEmpty(item.车辆修理费报销金额)}</车辆修理费报销金额>
            <停车过来费报销金额>${changeNullToEmpty(item.停车过来费报销金额)}</停车过来费报销金额>
            <其它报销金额>${changeNullToEmpty(item.其它报销金额)}</其它报销金额>
            <报销金额总合计>${item.报销金额总合计}</报销金额总合计>
            <报销金额总合计大写>${item.报销金额总合计大写}</报销金额总合计大写>
            <上传票据>${item.上传票据}</上传票据>
            <单据张数>${changeNullToEmpty(item.单据张数)}</单据张数>
        </集团本部_公司车辆费用报销单>
                    `;
            xml += `
                       </FormData>
